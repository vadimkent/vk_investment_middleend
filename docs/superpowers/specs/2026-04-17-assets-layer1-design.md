...
This is the **first of two layers** for the assets screen:

1. **List** — paginated table + `asset_type` filter (this spec).
2. **Mutations** — create (FAB + form drawer), edit (dialog), delete (with force-delete fallback).

Scope boundaries for L1:
...
|--------|------|------|-------------|
| GET | `/screens/assets` | yes | Assets screen component tree. Reads optional query `asset_type`, `offset` so deep-linking via URL params yields a coherent initial render. |
| GET | `/actions/assets/list` | yes | Returns an `ActionResponse{replace, target_id:"assets-section", tree}` that re-renders the filter + table + pagination. Idempotent read. Reads query `asset_type`, `offset`. |
...
- `X-Platform` — only `web` is meaningfully supported in L1. Other platforms receive the same web tree.

Downstream call: `GET /v1/assets?size=10&sort=ticker&order=desc[&asset_type=<v>][&offset=<n>]`.
...
All interactive controls live inside the `assets-section` subtree, which is the only part that gets replaced.
    ...
    column assets-section (gap=sm)              ← replaceable subtree
      form assets-filter-form
        row assets-filter-row widths=["240px","1fr"]
          ...
          spacer filter-spacer size="none"     ← fills the 1fr column so the select stays left-aligned
      table assets-table
        columns:
      ...
      row assets-pagination widths=["auto","1fr","auto"] gap="md"
        button pagination-prev
          props:
            label: i18n "assets.pagination.prev"
            variant:"secondary"
            style:"ghost"
            disabled: offset == 0
          actions: [{
        ...
        text pagination-info
          content: i18n "assets.pagination.page_of" { current, total_pages }
          size:sm, color:muted
        button pagination-next
          props:
            label: i18n "assets.pagination.next"
            variant:"secondary"
            style:"ghost"
...
The pagination row is omitted when `total <= size` (nothing to paginate).
...
All cell strings are produced server-side.
...
| `price_provider` | value as-is (e.g. `TWELVE_DATA`), `"—"` when `null`, `"—"` when `is_complex` | — |
...
- `size` is always `10` (hardcoded for L1).
- `page = (offset / size) + 1` (1-based for display).
- `total_pages = ceil(total / size)`. When `total == 0`, the pagination row is omitted (empty state handles it).
- `prevOffset = max(0, offset - size)`.
- `nextOffset = offset + size`.
- The `pagination-info` text uses the `assets.pagination.page_of` key with `{current}` and `{total}` (= `total_pages`) placeholders.
...
| Invalid query param (`asset_type` not in `{"", STOCK, ETF, CRYPTO, BOND}`, `offset` non-integer or negative) | 400 | `{"error":{"code":"BAD_REQUEST","message":"..."}}` |

The frontend shows a retry UI for 5xx and issues a fresh GET when the user retries.
...
```
internal/assets/
  client.go              - Client.ListAssets(ctx, auth, params) (ListResult, error)
  client_test.go
  types.go               - Asset, ListParams, ListResult{Assets, Total, Size, Offset}
  builder.go             - BuildScreen(result, params, lang) components.Component
                           BuildAssetsSection(result, params, lang) components.Component
  builder_test.go
  get_usecase.go         - Execute(ctx, auth, params, lang) (components.Component, error)
  get_usecase_test.go
  handler.go             - GET /screens/assets
  handler_test.go
...
- `builder.go` — SDUI tree construction. `BuildAssetsSection` is reused by both handlers (the screen handler wraps it in `screen` + root `column`; the list handler returns it directly inside an `ActionResponse`).
...
- [ ] `asset-type-select` carries `default_value` equal to the `asset_type` query param (or `""` when absent).
- [ ] The action on `asset-type-select` is `{trigger:"change", type:"submit", method:"GET", endpoint:"/actions/assets/list", target_id:"assets-filter-form", loading:"section"}`.
- [ ] `assets-table` has one `table_row` per asset; each row contains 6 `text` cells in order ticker/name/type/currency/complex/price_provider.
- [ ] `is_complex=true` → cell `"✓"`; `is_complex=false` → cell `"—"`.
- [ ] `price_provider` renders the value; `null` or `is_complex=true` → `"—"`.
- [ ] When `total <= size`, the `assets-pagination` row is omitted.
- [ ] When emitted, pagination has `pagination-prev` (disabled when `offset==0`), `pagination-info` with localized `Page X of Y`, and `pagination-next` (disabled when `offset+size >= total`).
- [ ] Pagination buttons carry `reload` actions with `asset_type` baked into the URL and the correct target offset.
- [ ] `GET /actions/assets/list` with a valid JWT returns `ActionResponse{action:"replace", target_id:"assets-section", tree:<assets-section>}`.
- [ ] Invalid `asset_type` (not in the accepted set) → `400 BAD_REQUEST`. Empty string is valid.
- [ ] Invalid `offset` (non-integer or negative) → `400 BAD_REQUEST`.
- [ ] Empty list without filter → `assets-empty` with keys `assets.empty_title` / `assets.empty_subtitle`.
- [ ] Empty list with filter → `assets-empty` with keys `assets.empty_filtered_title` / `assets.empty_filtered_subtitle`.
- [ ] Backend 5xx → `502 BACKEND_ERROR`; backend 401 → `401 unauthorized` with redirect.
- [ ] All user-visible strings resolve via i18n (`en` / `es`); no hardcoded literals in the response.
